  ...
  def pull[Bytes](input: Input[Bytes], receiver: Receiver[Input[Bytes], Bytes]): Input[Bytes] = {
    ...
    def decodeExtra(input: In, info: Int, uLong: Long): In =
      info match {
        ...
        case 24 ⇒
          uLong.toInt match {
            ...
            case x ⇒
              ...
              throw new Error.InvalidCborData(input, msg)
          }
        ...
        case x           ⇒ throw new Error.Unsupported(input, s"CBOR major type 7 code $x is unsupported by this decoder")
      }
    ...
    if (in.hasBytes(1)) {
      ...
      val uLong = if (info >= 24) {
        if (info <= 27) {
          ...
          } else throw new Error.InsufficientInput(in, byteCount.toLong)
        } else if (info == 31 && 2 <= majorType && majorType <= 7 && majorType != 6) 0L // handled specially
        else throw new Error.InvalidCborData(in, s"Additional info [$info] is invalid (major type [$majorType])")
